...
# ══════════════════════════════════════════════════════════
def get_client():
    """Load Anthropic client — reads key fresh every call."""
    key = ""
    ...
    # Strip whitespace/quotes that may have been accidentally included
...
def page_settings():
    ...
    with col2:
        ...
        if key and key.startswith("sk-ant-"):
            st.success(f"✅ Valid key found — starts with `{key[:20]}...` — length: {len(key)} chars")
        elif key:
            st.error(f"❌ Key found but looks wrong — starts with `{key[:20]}` — make sure it starts with `sk-ant-`")
        else:
            st.error("❌ No API key found at all")
